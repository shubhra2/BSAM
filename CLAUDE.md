...
- **Node / NVM & Wasp PATH Discovery**:
  If shell commands fail with `wasp: command not found` or `npx: command not found` due to NVM environments, prepend the user's NVM and local binary directories to `PATH` dynamically:
  ```bash
